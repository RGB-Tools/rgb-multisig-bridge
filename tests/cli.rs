...
#[test]
fn help() {
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_rgb-multisig-bridge"));
    cmd.arg("--help")
        .assert()
        .success()
...
fn version() {
    ...
    cmd.arg("--version")
        .assert()
        .success()
        .stdout(predicate::str::contains("rgb-multisig-bridge"));
}
...
"#;
    fs::write(storage_path.join("config.toml"), config_content).unwrap();
    let mut child = std::process::Command::new(env!("CARGO_BIN_EXE_rgb-multisig-bridge"))
        .arg(storage_path.to_str().unwrap())
        .arg("--daemon-listening-port")
        .arg("0")
        .stdout(Stdio::piped())
